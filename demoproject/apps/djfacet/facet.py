...
class Facet(object):
    ...
    def __init__(self, uniquename, name, originalModel, originalAttribute, displayAttribute=None, ordering=None,
                 behaviour=None, hierarchyoptions=None, number_opts=None, mask=None, customvalues=None, mptt=False,
                 explanation="", exclude=False, group=[], show_singlefacet_header=True):
    ...
    def create_allfacetvalues(self):
        ...
        if self.customvalues:
            for x in self.customvalues:
                ...
                v = FacetValue(name=x, facet=self, customvalues=self.customvalues[x], number_opts=self.number_opts,
                               show_singlefacet_header=self.show_singlefacet_header)
                self.add_facetvalue(v)

        # **case2:	TODO: THIS HASN'T BEEN REVISED IN A WHILE
        elif self.hierarchyoptions:
            values_list = self.originalModel.objects.exclude(id__in=[x.id for x in self.exclude]).values_list(
                self.originalAttribute, flat=True)
            ...
            if self.hierarchyoptions.get('alpha'):
                ...
                for x in lookup_choices:
                    v = FacetValue(name=x, facet=self, hierarchytype='alpha', number_opts=self.number_opts,
                                   show_singlefacet_header=self.show_singlefacet_header,
                                   mask=self.mask)    # father=0 implied
                    self.add_facetvalue(v)
            ...
            if self.hierarchyoptions.get('range'):
                ...
                for x in rangelist:
                    v = FacetValue(name=x[0], facet=self, hierarchytype='range', hierarchyextra=x[1],
                                   number_opts=self.number_opts, show_singlefacet_header=self.show_singlefacet_header,
                                   mask=self.mask)
                    self.add_facetvalue(v)
        ...
        # 2010-11-09 : new ==> MPTT models // in this case EXCLUDE doesn't work
            ...
            def helper_children(MPTTInstance, afacet, facetValueFather, rootfather):
                ...
                newValue = FacetValue(name=MPTTInstance.name, facet=afacet,
                                      father=facetValueFather, rootfather=rootfather, mpttoriginalID=MPTTInstance.id,
                                      mptt_treeID=MPTTInstance.tree_id)
            ...
            for x in top_level:
                newValue = FacetValue(name=x.name, facet=self, father=None, rootfather=None, mpttoriginalID=x.id,
                                      mptt_treeID=x.tree_id)
        ...
        # while the displayAttribute is the original field that gets displayed
            ...
            values_list = self.originalModel.objects.exclude(id__in=[x.id for x in self.exclude]).values_list(
                self.originalAttribute, self.displayAttribute)
            ...
            for x in values_list:
                if not self.get_facetvalue_from_displayname(
                        x[1]):     #extra check for non-deterministic displayAttribute
                    v = FacetValue(name=x[0], displayname=x[1], facet=self, number_opts=self.number_opts,
                                   show_singlefacet_header=self.show_singlefacet_header,
                                   mask=self.mask) # father=0 implied
                    self.add_facetvalue(v)
        ...
        # **case5:
            ...
            values_list = self.originalModel.objects.exclude(id__in=[x.id for x in self.exclude]).values(
                self.originalAttribute).annotate(howmany=Count(self.originalAttribute))
            # values_list = sorted(list(set(values_list)))
            for x in values_list:
                v = FacetValue(name=x[self.originalAttribute], facet=self, number_opts=self.number_opts,
                               show_singlefacet_header=self.show_singlefacet_header, mask=self.mask,
                               howmany=x['howmany']) # father=0 implied
                self.add_facetvalue(v)
    ...
    def get_facetvalue_from_id(self, valueid):
        ...
        if DJF_URL_AS_NUMBERS:
            try:
                ...
                djfacetlog(
                    "Facet.py>> DJF_URL_AS_NUMBERS is True but can't convert facetvalue ID to int! Error: %s" % (e),
                    True)
        for x in self.facetvalues:
            if x.id == valueid:
                return x
