...
class DbCache(object):
    ...
    def _cacheOneFacet(self, facet, resulttype, ENFORCE=False, values_to_cache=None):
        ...
        try:
            cached_fquery_old = CachedFacetQuery.objects.get(facet=facet.uniquename,
                                                             resulttype=resulttype['uniquename'],
                                                             queryargs=exploded_queryargs)
        except:
            cached_fquery_new = CachedFacetQuery(facet=facet.uniquename, resulttype=resulttype['uniquename'],
                                                 queryargs=exploded_queryargs)
        ...
        # delete the previous contents first
            ...
            djfacetlog(
                "cacheDB>> ENFORCE parameter not found (it's a new object or ENFORCE is set to false) => not deleting previously saved values")
        ...
        # ___________
            ...
            djfacetlog("cacheDB>> Facet= *%s*	 Result= *%s* ........... Object already Cached!" % (
            facet.uniquename, resulttype['uniquename']))
            return False
    ...
    def _inner_cache(self, cached_fquery, values_to_cache, facet, resulttype):
        ...
        for fv in values_to_cache:
            counter += 1
            try:
                djfacetlog("cacheDB>> Facet= *%s*	 Result= *%s*  Caching value %d of %d -- name[%s] count[%s]" % (
                facet.uniquename, resulttype['uniquename'], counter, counterTot, str(fv.name), str(fv.howmany)))
            except:
                djfacetlog(
                    "cacheDB>> Facet= *%s*	 Result= *%s*  Caching value %d of %d ... name couldn't be printed" % (
                    facet.uniquename, resulttype['uniquename'], counter, counterTot))
            try:
                ...
                if fv and facet.mptt:
                    if showonly_subs:
                        ...
                        if test_fv.id == fv.father:
                            ...
                            break
                    fv_copy = copy.copy(
                        fv)     #make a copy, so we NEVER update the static FM object (and it's thread-safe!)
                    fv_copy.howmany = val_obj.count
                    fv_copy.subspreview = [facet.get_facetvalue_from_id(x) for x in val_obj.mpttsubs.split("**$**") if
                                           val_obj.mpttsubs]
    ...
    def _emptyCacheForFacetQuery(self, facet, resulttype, queryargs=None):
        ...
        if queryargs == "only_with_queryargs":    # special case
            try:
                obj_list = CachedFacetQuery.objects.filter(Q(facet=facet.uniquename),
                                                           Q(resulttype=resulttype['uniquename']), ~Q(queryargs=None))
            ...
            for obj in obj_list:
                ...
                djfacetlog(
                    "\n!!!!!!!!!... removing all cached objects .........\n facet[%s] resType[%s] queryargs=[NOT NULL]" % (
                    facet.uniquename, resulttype['uniquename'],))
                obj.delete()
            return True
            ...
            try:
                obj_list = CachedFacetQuery.objects.filter(Q(facet=facet.uniquename),
                                                           Q(resulttype=resulttype['uniquename']))
            except:
            ...
            for obj in obj_list:
                ...
                #	obj.queryargs.delete()
                djfacetlog(
                    "\n!!!!!!!!!... removing all cached objects .........\n facet[%s] resType[%s] queryargs=[any]" % (
                    facet.uniquename, resulttype['uniquename'],))
                obj.delete()
            return True
        ...
        try:
            cached_fquery_old = CachedFacetQuery.objects.get(facet=facet.uniquename,
                                                             resulttype=resulttype['uniquename'], queryargs=queryargs)
        except:
            return "there is not such facet to delete in the cached DB table"
        djfacetlog(
            "\n!!!!!!!!!!!!!!... removing all cached objects .........\n facet[%s] resType[%s] queryargs=[%s]" % (
            facet.uniquename, resulttype['uniquename'], str(queryargs)))
...
def cacheHTMLpage(text, pagename, extraargs="", ENFORCE=False):
    ...
    if c and ENFORCE:
        ...
        djfacetlog(
            "CACHEHTMLPAGE: The page [%s, %s] is already cached. Use ENFORCE to recache it." % (pagename, extraargs))
    else:
